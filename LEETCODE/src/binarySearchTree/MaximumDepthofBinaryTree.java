...
class Tree1{
	...
	public boolean PathSumHelper(TreeNode root, int sum, int target){
		...
		sum += root.val;//遍历过程中累加节点值
		if(root.left == null && root.right == null){//leaf才检查结果是否相等
			if(sum == target){
				return true;
			}else{
				return false;
	...
	public int getMin(TreeNode root){
		if(root == null){ return Integer.MAX_VALUE; }//防空指针
		if(root.left == null && root.right == null){
			return 1;
		}
	...
	public boolean isSameTree(TreeNode p, TreeNode q){
		...
		if(p != null && q != null){
			if(p.val == q.val){//两树同一节点都有相等值，进行两树左节点判断和两树右节点判断
